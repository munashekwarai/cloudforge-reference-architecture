...
CloudForge separates adapters from domain decisions and persistence. Inputs cross validation boundaries before reaching the core. The core returns explicit evidence rather than hiding uncertainty. Storage is replaceable, and network/provider side effects are injectable so tests remain deterministic.
